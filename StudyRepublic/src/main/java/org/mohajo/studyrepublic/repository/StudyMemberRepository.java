...
//	List<StudyMember> findTutorActivityByStudyMemberId(StudyMemberId studyMemberId);
	...
	/*회원의 모든 스터디 정보 가져오기*/
	@Query(value="select * from study_member sm join study_view sv on sm.study_id=sv.study_id where sm.id= :id",nativeQuery=true)
	List<StudyMember> findAllStudyByList(String id);
